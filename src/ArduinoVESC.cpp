...
void ArduinoVESC::setSerialPort(HardwareSerial* port)
{
  serialPort = port;
}

void ArduinoVESC::setDebugPort(Stream* port)
{
  debugPort = port;
}

int ArduinoVESC::receiveUartMessage(uint8_t * payloadReceived) {
...
bool ArduinoVESC::unpackPayload(uint8_t * message, int lenMes, uint8_t * payload) {
...
int ArduinoVESC::packSendPayload(uint8_t * payload, int lenPay) {
...
bool ArduinoVESC::processReadPacket(uint8_t * message) {
  ...
  switch (packetId){
    case COMM_GET_VALUES: // Structure defined here: https://github.com/vedderb/bldc/blob/43c3bbaf91f5052a35b75c2ff17b5fe99fad94d1/commands.c#L164
      ...
      data.avgId             = buffer_get_float32(message, 100.0, &ind);
      data.avgIq             = buffer_get_float32(message, 100.0, &ind);
...
bool ArduinoVESC::getVescValues(void) {
...
void ArduinoVESC::setCurrent(float current) {
  int32_t index = 0;
  uint8_t payload[5];
...
void ArduinoVESC::setBrakeCurrent(float brakeCurrent) {
  int32_t index = 0;
  uint8_t payload[5];
...
void ArduinoVESC::setRPM(float rpm) {
  int32_t index = 0;
  uint8_t payload[5];
...
void ArduinoVESC::setDuty(float duty) {
  int32_t index = 0;
  uint8_t payload[5];
...
void ArduinoVESC::serialPrint(uint8_t * data, int len) {
  if(debugPort != NULL){
    for (int i = 0; i <= len; i++)
    {
...
void ArduinoVESC::printVescValues() {
  if(debugPort != NULL){
